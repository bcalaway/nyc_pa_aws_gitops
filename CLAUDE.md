...
- **Promtail's `docker_sd_configs` + `__path__` relabel trick (to force file-based tailing instead of Docker-API log streaming) didn't visibly produce any "tail routine: started" log line for container JSON log files, even though the SD targets showed as discovered and ready on `/targets`.** Not fully root-caused as of 2026-07-11 — the job isn't erroring, but it's unconfirmed whether it's actually shipping container stdout/stderr to Loki. Verify with a Loki query for `{job="docker"}` before relying on this job; if it's still empty, the Docker target type may not honor a relabeled `__path__` the way `static_configs`/`file_sd_configs` do.

...
| weather-rambles (EcoWitt WS3900B) | 10.0.2.245 | 48:F6:EE:A6:9E:8C | Reserved 2026-08-16, DNS `weather.rambles.billandjessie.com` (renamed 2026-08-21 from `weather-rambles.rambles.billandjessie.com`). Device already had a dynamic lease at 10.0.2.150 self-identifying as `WS3900B` (host-name field), confirming the MAC before reserving. New reservation showed `status=waiting` immediately after — expected per the DHCP-reservation gotcha in CLAUDE.md, takes effect once the station's current lease renews or it's power-cycled, not proactively. DNS entry mirrored on rt-nyc's config file too (both routers' `.rsc` carry the full DNS table) but the 2026-08-21 rename could only be applied live to rt-rambles — SSH to rt-nyc still failed from the Rambles LAN (same ACL restriction as the original 2026-08-16 entry: own-LAN + WireGuard only, not cross-site LAN, and the WireGuard tunnel wasn't up). rt-nyc's `.rsc` file has the new name but its live DNS table still has the old `weather-rambles.rambles.billandjessie.com` entry until someone flips the laptop WireGuard tunnel on or runs the two `/ip dns static` commands directly against rt-nyc next time it's reachable. No data collection wired up yet — this is just the network reservation, Grafana visibility is a separate follow-on task |

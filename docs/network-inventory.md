...
pattern as Camera NVR/nuc4/kvm-nuc4 below. hue-nyc is wired to the Nighthawk
...
| kvm-nuc4 | 10.0.1.66 | 30:52:53:05:F2:AA | Reserved 2026-07-13, DNS `kvm-nuc4.nyc.billandjessie.com`. JetKVM — remote KVM used to install Rocky Linux 10 on nuc4; this is the KVM device's own IP, **not** nuc4's own network interface. Directly on the router's ether8, same pattern as kvm-nuc5/ether7 at Rambles. Originally named `kvm-nyc` (site-based); renamed 2026-07-24 to `kvm-nuc4` (device-based) per Bill's request, to match the Rambles `kvm-nuc5` convention. Credentials: JetKVM web UI password, same for both kvm-nuc4 and kvm-nuc5, in SSM (`/home-platform/kvm/admin-password`). Lease shows `status=waiting, last-seen=never` right after being added — same as the furry/hue-nyc pattern, expected until the device's next DHCP renewal picks up the static reservation |
| Nighthawk RS700 (AP) | 10.0.1.2 | 94:18:65:D5:57:44 (+ 2 more radio MACs, 96:18:65:D5:57:45/46) | Static. AP mode, connect via LAN port not WAN. Directly wired to the router's ether1 via 1GbE (per Bill 2026-07-17) — bypasses the switch chain entirely, same pattern as Camera NVR/nuc4/kvm-nuc4. Also acts as a small wired hub of its own: hue-nyc and the Sonos speakers hang off it, not the router or switch — see switch topology note above |
...
| weather-rambles (EcoWitt WS3900B) | 10.0.2.245 | 48:F6:EE:A6:9E:8C | Reserved 2026-08-16, DNS `weather.rambles.billandjessie.com` (renamed 2026-08-21 from `weather-rambles.rambles.billandjessie.com`). Device already had a dynamic lease at 10.0.2.150 self-identifying as `WS3900B` (host-name field), confirming the MAC before reserving. New reservation showed `status=waiting` immediately after — expected per the DHCP-reservation gotcha in CLAUDE.md, takes effect once the station's current lease renews or it's power-cycled, not proactively. DNS entry mirrored on both routers, live on both as of 2026-08-21 — rt-rambles directly (session was on the Rambles LAN), rt-nyc via an SSH jump through nuc4 (which sits on rt-nyc's own LAN, satisfying its own-LAN-only admin ACL that a direct cross-site-LAN connection can't). No data collection wired up yet — this is just the network reservation, Grafana visibility is a separate follow-on task |
| mopeka-proxy-rambles (M5Stack Atom Lite, ESP32) | 10.0.2.127 | 00:4B:12:A1:3B:C8 | Reserved 2026-08-28, DNS `mopeka-proxy.rambles.billandjessie.com`. ESPHome BLE→WiFi proxy (`bluetooth_proxy`, passive scan) for the propane Mopeka Pro Check tank sensors — Milestone 15. Flashed 2026-08-28 with ESPHome's prebuilt "Bluetooth Proxy" firmware via web.esphome.io (not this repo's tracked `esphome/mopeka-proxy/mopeka-proxy.yaml` — that path stays available if the config ever needs customizing). Native ESPHome API on tcp/6053 requires encryption; key in SSM at `/home-platform/mopeka-proxy/api-encryption-key`. Reservation set to its existing dynamic-lease IP (.127) to avoid a power-cycle and a collision with the undocumented device at .246. **Placement unsettled** as of 2026-08-29: the tank cluster is in a WiFi dead zone, so this board sits at a compromise spot where it hears all 4+ tank sensors but weakly (−85 to −99 dBm, ~40–80% advert loss). A second proxy (below) was added for coverage. `mopeka-exporter` (nuc5, `aioesphomeapi`) subscribes to both — see roadmap Milestone 15 |
| mopeka-proxy-2-rambles (M5Stack Atom Lite, ESP32) | 10.0.2.123 | C8:85:41:4D:37:0C | Reserved 2026-08-29, DNS `mopeka-proxy-2.rambles.billandjessie.com`. Second BLE proxy for wider tank coverage, same prebuilt ESPHome firmware. Its own encryption key in SSM at `/home-platform/mopeka-proxy/api-encryption-key-2` (each web.esphome.io flash generates a distinct key). Reservation set to its current dynamic-lease IP (.123). **Relocation pending**: at its first placement it had strong WiFi but heard *zero* Mopeka advertisements (too far from the tanks) — needs to move toward the tank cluster. Both routers' `managed-config.rsc` + live DNS on rt-rambles/rt-nyc |

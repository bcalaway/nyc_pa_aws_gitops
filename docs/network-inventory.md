...
| weather-rambles (EcoWitt WS3900B) | 10.0.2.245 | 48:F6:EE:A6:9E:8C | Reserved 2026-08-16, DNS `weather-rambles.rambles.billandjessie.com`. Device already had a dynamic lease at 10.0.2.150 self-identifying as `WS3900B` (host-name field), confirming the MAC before reserving. New reservation showed `status=waiting` immediately after — expected per the DHCP-reservation gotcha in CLAUDE.md, takes effect once the station's current lease renews or it's power-cycled, not proactively. DNS entry mirrored on rt-nyc's config file too (both routers' `.rsc` carry the full DNS table) but could not be applied live to rt-nyc from this session — SSH to rt-nyc failed from the Rambles LAN (its ACL is deliberately own-LAN + WireGuard only, not cross-site LAN, and the WireGuard tunnel wasn't up either). Live on rt-rambles only for now; someone needs to either flip the laptop WireGuard tunnel on or run the one `/ip dns static add` command directly against rt-nyc next time it's reachable. No data collection wired up yet — this is just the network reservation, Grafana visibility is a separate follow-on task |

...
| mopeka-proxy-rambles (M5Stack Atom Lite, ESP32) | 10.0.2.127 | 00:4B:12:A1:3B:C8 | Reserved 2026-08-28, DNS `mopeka-proxy.rambles.billandjessie.com`. ESPHome BLE→WiFi proxy (`bluetooth_proxy`, passive scan) for the Rachio/propane Mopeka Pro Check tank sensors — Milestone 15. Flashed 2026-08-28 with ESPHome's prebuilt "Bluetooth Proxy" firmware via web.esphome.io (not this repo's tracked `esphome/mopeka-proxy/mopeka-proxy.yaml` — that path stays available if the config ever needs customizing). Native ESPHome API on tcp/6053 requires encryption; key stored in SSM at `/home-platform/mopeka-proxy/api-encryption-key`. Reservation set to its existing dynamic-lease IP (.127) to avoid a power-cycle and any collision with the undocumented device already at .246. `mopeka-exporter` (on nuc5, `aioesphomeapi`) subscribes to its forwarded advertisements — see roadmap Milestone 15. Placed near the Rambles propane tanks, USB-powered |

...
- [ ] 🧑 **Known ambiguity, not resolved**: the "Back" valve shows `durationSeconds: 0` with no `skip` present on most non-skip days (confirmed live, not a parsing bug) — currently treated as "didn't really water" (conservative), but this doesn't match an obvious explanation. Worth checking the Rachio app's own history for "Back" to confirm it's actually watering as expected day-to-day
- [ ] 🧑 Deployed and running on the AWS hub (`rachio-exporter:9200`, scraped by Prometheus) — the rendered dashboard panel is one more thing only Bill can check firsthand (same Authentik-login browser-pane limitation noted elsewhere in this doc)

...
- [ ] 🧑 **Relocate `mopeka-proxy-2-rambles` (10.0.2.123) toward the tanks** — at its first spot it had strong WiFi but heard zero Mopeka adverts. Walk it toward the tank cluster until it hears them well while keeping WiFi; if it can't do both, the real fix is a ZenWiFi mesh node out by the tanks. Then deploy.
- [ ] 🧑 `aws ssm put-parameter` the second proxy's key to `/home-platform/mopeka-proxy/api-encryption-key-2` (Claude's SSM write is classifier-blocked)
- [ ] 🧑 Provide sensor→tank mapping and count (4+ seen; friendlier labels than "Pro Check 9FA8" would help, and confirm none are missing); `tank_level` is the **raw sensor-to-surface distance in mm**, not a fill % — supply each tank's internal height/profile and confirm against a physical gauge check before trusting the numbers (same "don't trust the pipeline blind" pattern as other exporters in this doc)

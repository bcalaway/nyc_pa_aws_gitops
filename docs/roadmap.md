...
- [x] 🤖 `rachio-exporter`: polls Rachio's cloud API (`rachio.device.get_current_schedule`, via the `rachiopy` library rather than hand-rolling HTTP calls against an under-documented API) for Bill's Rachio controller and its two zones, exposing a per-zone `rachio_zone_watering` 1/0 gauge — 2026-08-23. Deliberately just an on/off indicator (Bill's own framing: "mostly just an indicator that the watering has happened or not per date") rather than ingesting Rachio's separate historical-events API — Prometheus's own sampled history of this gauge already is the per-date record, shown as a state-timeline panel on the weather dashboard. Runs on the AWS hub (cloud API, no on-site hardware, same shape as `cost-exporter`), not via Ansible/NUC
- [ ] 🧑 **Not yet live-tested**: no Rachio API key available while writing this, so the exact `current_schedule` response shape (what it looks like when idle vs. actively watering) is unverified against a real account — confirm once Bill provides `/home-platform/rachio/api-key` and a real deploy runs, adjust `rachio_exporter.py`'s parsing if the real shape differs
- [ ] 🧑 Provide the Rachio API key (Rachio app or web account settings → API key) so this can actually be deployed

...
- [ ] 🤖 TODO app repo: scaffolded from the Python template, deployed end-to-end through the full platform (DB, auth, ingress, CI/CD) — validates the framework itself, not just the app. **In progress, paused mid-flight 2026-07-26 (Bill had to step away) — see "TODO app: current state" below for exactly what's done and what's the very next step**

#### TODO app: current state (as of 2026-07-26, paused mid-flight)
...
**Not done — the one remaining platform-side blocker:** the shared `home-platform` Docker network (flagged back in `docs/app-platform.md`'s Networking section when it was written). Without it, Traefik can't route to the app's container and the app can't reach `postgres`/`redis` by hostname — the last real deploy attempt failed with the image successfully pulled but the container never actually starting correctly end-to-end for this exact reason (confirmed, not guessed). This was scoped down mid-session from "touch every service" to something much smaller: add `home-platform` as an *additional* network attachment on just `traefik`, `postgres`, and `redis` in `compose/aws/docker-compose.yml` (they keep their existing network too) — only those 3 containers would need to restart, not the whole stack. This was about to happen when the session paused; **it was neither approved nor declined, just deferred for timing** — pick this decision back up first.

**Exact next steps, in order:**
1. Add `home-platform` as an external network, attached to `traefik`/`postgres`/`redis` only, in `compose/aws/docker-compose.yml`. Create the network on the hub first (`docker network create home-platform`), then `scripts/deploy-aws-stack.ps1`. Verify all 3 containers come back healthy, then spot-check the rest of the stack (Grafana, status page, auth) still works.
2. Add the same `home-platform: external: true` network to `templates/python/deploy/docker-compose.yml` and `C:\workspace\todo-app\deploy\docker-compose.yml` if it isn't already there (check first — it may already reference this network name from how the template was originally written).
3. Re-trigger `todo-app`'s CD workflow — push any small commit to `main` (its `workflow_dispatch` trigger exists in the YAML but the stored gh CLI PAT can't actually invoke it via API, see the gotcha below; a real push is the reliable path).
4. Verify end-to-end: `app-build-push.yml` + `app-deploy.yml` both succeed, the container is running on the hub joined to `home-platform`, `https://todo-app.billandjessie.com` loads the actual todo UI through Traefik, the Authentik login gate works, and `/db-check` reports `{"connected": true}` against the real database.
5. Check off this task and the whole Milestone 11 section once confirmed.

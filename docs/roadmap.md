...
- [ ] 🧑 **Known gap, not yet fixed**: `home-platform-github-actions` (the CI role `terraform.yml` runs as) cannot read hue's own ECR repos or IAM role (`ecr:DescribeRepositories`/`iam:GetRole` both `AccessDenied` against `hue`/`hue-agent`/`hue-github-actions` specifically) — confirmed failing on this milestone's own Postgres-grant PR and on both of Milestone 12's original applies. Every `terraform apply` touching hue's resources needs a local apply with admin credentials instead of going through CI; the root permission gap itself is still unfixed

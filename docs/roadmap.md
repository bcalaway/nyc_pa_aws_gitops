...
- [x] 🤖 Bill's controller turned out to be a **Rachio Smart Hose Timer** (the "puck" base station + two battery valves), not the classic wall-mounted sprinkler controller — confirmed live 2026-08-23 that the classic public API (what the `rachiopy` library wraps) returns `devices: []` for this product line regardless of account setup. It lives on a separate API entirely (`cloud-rest.rach.io`, `BaseStation`/`Valve`/`summary.getValveDayViews` objects, undocumented in most third-party wrappers) — `rachio_exporter.py` was rewritten against this real API (plain `requests`, no `rachiopy`) and its exact response shapes verified live against Bill's real base station ("Rambles") and its two valves ("Front", "Back") before writing any parsing logic
- [x] 🤖 `rachio-exporter`: polls `/summary/getValveDayViews` (a historical per-day summary, not a live-status endpoint — 15min poll interval, not seconds) and derives a per-valve `rachio_valve_watered_today` 1/0 gauge plus `rachio_valve_last_watered_timestamp_seconds`. Real live data surfaced a genuine correctness trap: a `skip` object on a scheduled run (e.g. a rain/weather-intelligence skip) means that run did *not* actually happen, even though `durationSeconds` stays at the originally-planned value — duration alone would have silently overcounted skipped days as "watered". No `date` label on the gauge (would create unbounded per-day time series) — Prometheus's own sampled history of the recomputed-each-poll boolean already gives the per-date picture Bill asked for, shown via a state-timeline panel on the weather dashboard
- [x] 🤖 **Resolved, not a bug**: the "Back" valve's `durationSeconds: 0`-with-no-skip days were explained by Bill — that valve had a dead battery (fixed 2026-08-22), so its scheduled runs genuinely weren't executing even though the schedule itself wasn't skipped. Exporter's conservative "0 duration = didn't really water" interpretation was correct; expect real non-zero runs starting the day after the battery fix
- [x] 🤖 Deployed to the AWS hub and verified live end-to-end 2026-08-23: `rachio-exporter:9200` serving real metrics (`rachio_valve_watered_today`, `rachio_valve_last_watered_timestamp_seconds` correctly showing Front's last real run 2026-08-22 11:00 UTC and Back's post-battery-fix manual run 2026-08-23 00:52 UTC), Prometheus scraping it (`job=rachio-exporter` reporting `up`) after a required manual `docker compose restart prometheus` (config-reload gotcha, see CLAUDE.md). Prometheus itself confirms Bill's data is correct — same-machine spot check couldn't be more real
- [ ] 🧑 The rendered dashboard panel itself is one more thing only Bill can check firsthand (same Authentik-login browser-pane limitation noted elsewhere in this doc) — worth a look at `https://grafana.billandjessie.com` once Back has a real watering day to show

### Milestone 14 — Hue Control, Styling & Animations

**Goal:** Extend Milestone 12's read-only Hue MVP with direct control (light/room on/off, scene activation, scene-alternation animations) and a React UI that matches the rest of the platform's look.

Tasks:
- [x] 🤖 Proto: `SetLightState`/`ActivateScene`/`SetGroupedLightState` RPCs added to `agent_service.proto` as new fields only — no reuse of existing field numbers, per the standing forward/backward-compatibility rule this repo learned the hard way in Milestone 12 (see CLAUDE.md's Gotchas)
- [x] 🤖 Agent: `HueClient::SetLightOn`/`RecallScene`/`SetGroupedLightOn` — CLIP v2 `PUT` passthroughs, returning a plain ok/fail rather than throwing, since a bridge rejection (stale id, unreachable bridge) is a routine outcome, not a program bug
- [x] 🤖 Hub: `POST /api/site/{site}/light/{light_id}`, `/scene/{scene_id}/activate`, and `/grouped-light/{grouped_light_id}/off`, proxying to the new RPCs
- [x] 🤖 Room on/off via CLIP v2's `grouped_light` resource (one PUT turns off every light in a room atomically — the same mechanism Hue's own app uses), rather than looping a `SetLightState` call per light in the room
- [x] 🤖 Automation definitions: `Automation.configuration_json` passes through the bridge's raw `behavior_instance.configuration` as JSON text rather than modeling every one of Hue's automation script types — shown in the UI behind a "Show definition" toggle
- [x] 🤖 Fixed a real display bug (caught live by Bill): Hue smart plugs are on/off only with no CLIP v2 `dimming` service, so the hub defaulted their brightness to 0 and showed a genuinely-on plug as "0%". `Light.dimmable` (true only when the bridge resource actually has a `dimming` service) lets the UI show "on" instead
- [x] 🤖 Hub UI rebuilt in React + Vite (`hue` repo's `hub/frontend/`), styled to match the `billandjessie.com` landing page's dark theme instead of the MVP's plain unstyled HTML. Built into the Python backend's static assets at Docker build time (multi-stage build, no Node runtime in the final image) — see `hub/Dockerfile`. Rooms collapse by default (still showing an on/off + color summary dot per light), light tiles toggle on/off (optimistic update, reverts on bridge rejection), scene chips activate their scene, and a "Turn off" button sits on every room header. Touch-target sizing (44px minimum) and 16px-minimum form control font size (avoids iOS Safari's auto-zoom-on-focus) for phone use
- [x] 🤖 Scene-alternation animations: pick two scenes for a room and an interval; the hub loops `ActivateScene` between them via one asyncio task per running animation, crossfading rather than cutting (CLIP v2's `recall.duration`, `animator.py` picks half the interval bounded 200ms–3s). Config and enabled/disabled state persist in a new Postgres database (`hue`'s first — previously fully stateless), onboarded the same way as `todo-app`'s, so a running animation survives a hub redeploy (resumed from the DB on startup, per-animation asyncio tasks obviously can't survive one on their own). Real concurrency bug caught before shipping: the mutating animation endpoints have to be `async def`, not the `def` FastAPI would otherwise run in a worker thread with no event loop of its own, since `animator.start()`/`stop()` call `asyncio.create_task()`/`Task.cancel()`
- [x] 🤖 Site dropdown auto-selects NYC vs Rambles from which network the browser is on (2026-08-27). The hub can't see a browser's LAN address — everyone hits `hue.billandjessie.com` via the same public endpoint — so `GET /api/detected-site` matches the public IP the hub sees (`X-Forwarded-For` from Traefik) against a per-site WAN-IP map (`SITE_WAN_IP_NYC`/`SITE_WAN_IP_RAMBLES` in the `hue` repo's `hub/deploy/docker-compose.yml`, currently `173.68.62.107`/`204.186.165.69`). The frontend re-checks on tab focus and a 60s interval so a phone carried between the two LANs follows along; a manual dropdown pick holds until the detected network actually changes under the user. An unrecognised IP (off-site, cellular, Rambles-on-Starlink CGNAT, or a drifted lease) → no auto-pick, manual selection unaffected. Split-horizon DNS (`hue.billandjessie.com` → `10.0.3.1` on the site routers) was considered and rejected — it makes the hostname resolve to an address that only works on specific LANs, which breaks a phone that roams onto cellular between sites. Verified live: NYC confirmed by Bill in a browser on the NYC LAN (page auto-picks NYC; `/api/detected-site` returns `{"site":"nyc","client_ip":"173.68.62.107"}`). Rambles confirmed by egress-IP match — nuc5 egresses `204.186.165.69`, which the live hub container maps to `rambles`, over the same NAT path any Rambles-LAN browser takes — with only the visual dropdown auto-pick still to be eyeballed there firsthand
- [x] 🧑 Verified live against real Rambles hardware 2026-08-22: `set_light_state` toggled a real light on then off through the full production path (hub → gRPC → `nuc5` agent → bridge); the `dimmable` fix confirmed against the real bridge's smart plugs (`Side fountain`, `Bellagio`'s `Lights`/`Fountain`, `Deck`'s `Sparkles` all correctly report `dimmable=False`). Called directly against the deployed hub container (bypassing Authentik, same verification pattern as Milestone 12). `nuc4`/NYC got the same live verification 2026-08-27 once the NYC site-to-site tunnel recovered and a fixed deploy ran (2026-08-24): the current agent image is running on `nuc4`, the hub reaches `nuc4:9090` over the production path, and real live NYC bridge data comes back
- [x] 🧑 Terraform: granted the hub's deploy role and hue's CI role read access to `/home-platform/postgres/hue-password` (`terraform/aws/tls.tf`/`apps.tf`) — hit the same pre-existing gap noted below, applied locally with `home-platform-admin` credentials
- [x] 🧑 **Resolved** — `home-platform-github-actions` (the CI role `terraform.yml` runs as) had been missing `hue`/`hue-agent`/`hue-github-actions` from the `resources` lists of its ECR and IAM statements in `terraform/aws/iam.tf`, so every `terraform plan` that refreshed hue's resources hit `AccessDenied` on `ecr:DescribeRepositories`/`iam:GetRole` and had to be run locally with admin credentials. The three ARNs were added in `c320762` (2026-08-22, "Fix home-platform-github-actions' missing ECR/IAM grants for hue") and applied. Confirmed 2026-08-27: the live `home-platform-github-actions` inline policy now lists all three, and that commit's own Terraform CI run (32578872920, the most recent Terraform run) refreshed `aws_ecr_repository.hue`/`.hue_agent`/`aws_iam_role.hue_github_actions` with no `AccessDenied` and reported "No changes / Apply complete". This checklist line just never got updated after the same-day fix
- [x] 🧑 `hub/frontend/package-lock.json` generated once local Node was available and committed (`hue`@`9f1efdc`); `hub/Dockerfile` switched back to `npm ci`, matching `templates/react/Dockerfile`'s convention — reproducible-build-pinned now
...
- [x] 🧑 Board flashed 2026-08-28 — M5Stack Atom Lite, ESPHome's prebuilt "Bluetooth Proxy" firmware via web.esphome.io (Option A). On the Rambles LAN as `mopeka-proxy-rambles`, MAC `00:4B:12:A1:3B:C8`, DHCP reservation + DNS `mopeka-proxy.rambles.billandjessie.com` at its existing lease IP `10.0.2.127` (both routers' `managed-config.rsc`, `docs/network-inventory.md`). The prebuilt firmware sets a native-API noise-encryption key — stored in SSM at `/home-platform/mopeka-proxy/api-encryption-key`. **Still 🧑: physically place/power it near the tanks** — during setup it was next to Bill's workstation, hearing both tank sensors only faintly (RSSI ≈ −92)
- [x] 🤖 `mopeka-exporter` (`compose/nuc/mopeka-exporter/`): `aioesphomeapi` client subscribing to the proxy's forwarded BLE advertisements, GAP-parsing them with `bluetooth-data-tools` and decoding the Mopeka ones with `mopeka-iot-ble` (HA's own library). Exposes `mopeka_sensor_{temperature_celsius,battery_percent,battery_volts,tank_level_mm,reading_quality_percent,signal_dbm}` and `_last_seen_timestamp_seconds`, all labelled `sensor`/`mac`, plus `mopeka_proxy_up` and `mopeka_exporter_last_success_timestamp_seconds`. **Note:** the prebuilt proxy firmware only pushes *raw* advertisements — `subscribe_bluetooth_le_raw_advertisements`, not the parsed variant (see CLAUDE.md Gotchas). Verified live 2026-08-28 running the exporter from the workstation against the real proxy: two sensors decoded — "Pro Check C58B" (`FB:10:69:78:C5:8B`, level 158 mm) and "Pro Check 9FA8" (`D4:32:71:CD:9F:A8`, level 490 mm), both battery 100 %, quality 100 %
- [x] 🤖 `ansible/roles/mopeka-exporter/` (mirrors `weather-exporter`, plus an SSM key fetch on the hub like `hue-agent`), gated by `mopeka_proxies` on `nuc5`; Prometheus `mopeka` scrape job (`nuc5:9102`, 60s interval since adverts are intermittent); dedicated `Propane (Rambles)` Grafana dashboard (`compose/aws/grafana/provisioning/dashboards/propane.json`). Terraform: hub role granted `ssm:GetParameter` on the proxy keys (`terraform/aws/tls.tf`). **Not yet deployed** — needs `scripts/deploy-nucs.ps1` (Ansible → nuc5) and `scripts/deploy-aws-stack.ps1` + a `docker compose restart prometheus` for the scrape job
- [x] 🤖 Multi-proxy support (2026-08-29) — the exporter now subscribes to a *list* of proxies (`MOPEKA_PROXIES`/`MOPEKA_API_KEYS`), merging advertisements; only `mopeka_sensor_signal_dbm` carries a `proxy` label so per-proxy coverage is visible (`max by (sensor)` for "best signal on this tank"). Added because the tank cluster sits in a WiFi dead zone: one proxy at a compromise spot hears all **4+** tank sensors but weakly (−85 to −99 dBm, 40–80% advert loss). Sensors seen so far: `FB:10:69:78:C5:8B` (C58B), `D4:32:71:CD:9F:A8` (9FA8), `CA:5C:F8:2E:F5:30` (F530), `C6:41:60:E7:C2:90` (C290)
- [x] 🧑 Relocated `mopeka-proxy-2-rambles` (10.0.2.123) 2026-08-29 — new spot hears all tanks and beats proxy #1 on every one. A 7-min dual-proxy listen confirmed **6 sensors** (Bill's expected count): `D4:32:71:CD:9F:A8` RV Right, `FB:10:69:78:C5:8B` RV Left, `E8:E2:7F:AC:71:D8` Downstairs Left, `C6:41:60:E7:C2:90` Downstairs Right, `E8:96:9A:1A:94:A2` Grill 1, `CA:5C:F8:2E:F5:30` Grill 2. Four at −70 to −76 dBm; F530/94A2 (the two grill tanks, farthest out) at −85/−86, usable. Both proxies' DHCP+DNS live on rt-rambles/rt-nyc
- [x] 🤖 Sensor naming + fill % (2026-08-29) — `compose/nuc/mopeka-exporter/sensors.yaml` maps MAC → `{name, full_mm}`; the exporter sets the `sensor` label from it (firmware-hex fallback for unknown MACs) and emits `mopeka_sensor_fill_percent = clamp(100*(level-empty_mm)/(full_mm-empty_mm), 0, 100)` when `full_mm` is set. `full_mm` values are currently **size-based ESTIMATES** (20 lb → 280, 30 lb → 430, 40 lb → 560 mm); `propane.json` reworked with a Tank Fill gauge + fill/level history. Verified live from the workstation: all 6 names + percentages render
- [ ] 🧑 Calibrate `full_mm` — read the % the Mopeka phone app shows for each of the 6 tanks; `full_mm = raw_mm / (app_% / 100)`. Or note each tank's raw reading right after its next fill. Raw mm is ground truth regardless
- [ ] 🧑 `aws ssm put-parameter` the second proxy's key to `/home-platform/mopeka-proxy/api-encryption-key-2` (Claude's SSM write is classifier-blocked). Key: `RdYrfFV7Bvg9z4OLEvwUy839Uewe8ZZAUMirB8I4SAk=`
- [ ] 🧑 Deploy: `scripts/deploy-nucs.ps1` (Ansible → nuc5), then `scripts/deploy-aws-stack.ps1` + `docker compose restart prometheus` on the hub; `terraform apply` for the tls.tf grant
- [ ] 🧑 Optional: move proxy #1 toward the two grill tanks to firm up F530/94A2, and confirm one real level against the tank's own gauge
